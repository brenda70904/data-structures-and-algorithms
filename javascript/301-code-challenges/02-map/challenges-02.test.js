...
const extractAbilities = (arr) => {
  let result = arr.map(name => name.ability.name);
  return result;
};
...
const extractStats = (arr) => {
  return arr.map((stats)=>{
    return stats = {name: stats.stat.name, total: stats.effort + stats.baseStat
    };
  });
};
